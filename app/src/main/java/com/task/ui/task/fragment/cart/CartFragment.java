...
public class CartFragment extends BaseFragment<FragmentCartBinding> implements UpdateTotalPriceListener {
    ...
    protected void initializeObject() {
        ...
        HashSet<Cart> hashSet = new HashSet(cartArrayList);
        ArrayList<Cart> newArrayList = new ArrayList<>();

        for(Cart strNumber : hashSet)
            newArrayList.add(strNumber);


        binding.totalPriceTextView.setText(totalAmount+" \u20B9");
        cartAdapter = new CartAdapter(getActivity(),newArrayList,this);
    ...
    @Override
    public void onUpdate(double totalPrice, String itemId) {
        String priceString = binding.totalPriceTextView.getText().toString();
        String[] split = priceString.split(" \u20B9");

        double newTotalAmount = totalPrice + (int)split[0].charAt(0);

        binding.totalPriceTextView.setText(newTotalAmount+" \u20B9");
        cartAdapter.addOrUpdateSingleItem(cartHelper.getSingleRowsFromFirstTable(itemId));
    }
}